    ...
    try {
        ...
        return {
            ...
            posts: (user.edge_owner_to_timeline_media?.edges || []).map(edge => ({
                ...
                date: edge.node.taken_at_timestamp ? new Date(edge.node.taken_at_timestamp * 1000).toISOString() : null,
...
module.exports = async (req, res) => {
    ...
    try {
        ...
        if (url.startsWith('/api/instagram/')) {
            const username = extractUsername(decodeURIComponent(url.split('/api/instagram/')[1]));
        ...
        if (url === '/api/library') {
            ...
            if (query.search) filtered = filtered.filter(i => i.caption && i.caption.toLowerCase().includes(query.search.toLowerCase()));
            ...
            return res.json({
                ...
                pagination: { page: page, limit: limit, total: filtered.length, totalPages: Math.ceil(filtered.length / limit) || 1 }
            });
        }
        ...
        if (url === '/api/scraper/run' && req.method === 'POST') {
            ...
            for (var i = 0; i < global.scraperConfig.accounts.length; i++) {
                var account = global.scraperConfig.accounts[i];
                var data = await getInstagramData(account.username);
                if (data.success && !data.profile.isPrivate) {
                    var newCount = addToLibrary(data.posts, account.username);
            ...
            return res.json({ success: true, results: results, totalNewPosts: totalNewPosts, librarySize: global.mediaLibrary.length });
        }
